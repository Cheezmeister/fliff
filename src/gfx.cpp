...
void render(GameState& state, u32 ticks)
{
    ...
    {
      FliffCapsule& c = state.capsules[i];
      if (c.active)
      {
        ...
        set_uniform(renderstate.shaders.capsule, "triggered", c.triggered ? 1 : 0);
        set_uniform(renderstate.shaders.capsule, "fuse", (float)c.fuse);
